...
def call_gemini_pro_vision(messages, objective):
    ...
    if config.verbose:
        print(
            "[Gen AI OS][call_gemini_pro_vision]",
        )
    # sleep for a second
    time.sleep(1)
    try:
...
async def call_gpt_4_vision_preview_labeled(messages, objective, model):
    ...
    try:
        ...
        content = clean_json(content)
        assitant_messages = {"role": "assitant", "content": content}
        ...
        for operation in content:
            ...
            if operation.get("operation") == "click":
                ...
                if config.verbose:
                    print(
                        "[Gen AI OS][call_gpt_4_vision_preview_labeled] label",
                        label,
                    )
                ...
                if config.verbose:
                    print(
                        "[Gen AI OS][call_gpt_4_vision_preview_labeled] coordinates",
                        coordinates,
                    )
                image = Image.open(
                ...
                if config.verbose:
                    print(
                        "[Gen AI OS][call_gpt_4_vision_preview_labeled] click_position_percent",
                ...
                if config.verbose:
                    print(
                        "[Gen AI OS][call_gpt_4_vision_preview_labeled] new click operation",
                        operation,
                    )
                processed_content.append(operation)
            else:
                if config.verbose:
                    print(
                        "[Gen AI OS][call_gpt_4_vision_preview_labeled] .append none click operation",
                        operation,
                    )
            ...
            if config.verbose:
                print(
                    "[Gen AI OS][call_gpt_4_vision_preview_labeled] new processed_content",
                    processed_content,
                )
            return processed_content
...
async def call_claude_3_with_ocr(messages, objective, model):
    ...
    try:
        ...
        if config.verbose:
            print(
                f"{ANSI_GREEN}[Self-Operating Compiuter]{ANSI_BRIGHT_MAGENTA}[{model}] content: {content} {ANSI_RESET}"
            )
...
def gpt_4_fallback(messages, objective, model):
    ...
    system_prompt = get_system_prompt("gemini-pro-vision", objective)
