...
from .exceptions import ModelNotRecognizedException  # Relative import for exceptions module
import platform
...
def main(model, terminal_prompt, voice_mode=False, verbose_mode=False):
    """
    Main function for the Gen AI OS.
    ...
    if terminal_prompt:  # Skip objective prompt if it was given as an argument
        ...
        print(
            f"{ANSI_GREEN}[Gen AI OS]{ANSI_RESET} Listening for your command... (speak now)"
        )
        try:
        ...
        print(
            f"[{ANSI_GREEN}Gen AI OS {ANSI_RESET}|{ANSI_BRIGHT_MAGENTA} {model}{ANSI_RESET}]\n{USER_QUESTION}"
        )
    ...
    while True:
        if config.verbose:
            print("[Gen AI OS] loop_count", loop_count)
        try:
            ...
            print(
                f"{ANSI_GREEN}[Gen AI OS]{ANSI_RED}[Error] -> {e} {ANSI_RESET}"
            )
            break
        except Exception as e:
            print(
...
def operate(operations, model):
    if config.verbose:
        print("[Gen AI OS][operate]")
    for operation in operations:
        if config.verbose:
            print("[Gen AI OS][operate] operation", operation)
        # wait one second
        time.sleep(1)
        ...
        operate_detail = ""
        if config.verbose:
            print("[Gen AI OS][operate] operate_type", operate_type)
        ...
        if operate_type == "press" or operate_type == "hotkey":
            ...
            print(
                f"{ANSI_GREEN}[Gen AI OS]{ANSI_RED}[Error] unknown operation response :({ANSI_RESET}"
            )
            print(
                f"{ANSI_GREEN}[Gen AI OS]{ANSI_RED}[Error] AI response {ANSI_RESET}{operation}"
            )
            return True
