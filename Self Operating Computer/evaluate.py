...
SCREENSHOT_PATH = os.path.join('screenshots','screenchot.png')

def supports_ansi():
    """
    Check is the terminal supports ANSI escape codes
    """
...
def get_test_model():
    parser = argparse.ArgumentParser(
        description="Run the self-operating-computer with a specific model."
    )
...
def main():
    ...
    for objective, guideline in TEST_CASE.items():
    ...
    print(
        f"{ANSI_BRIGHT_MAGENTA}[EVALUATION COMPLETER]{ANSI_RESET} {passed} test{'' if passed == 1 else 's'} passed, {failed} test{'' if failed == 1 else 's'} failed"
    )            
